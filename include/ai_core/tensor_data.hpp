 ...
 * buffer (`TypedBuffer::dataType()`). Shape axes remain in model/backend order.
 */
struct Tensor {
  std::string name;
  TypedBuffer buffer;
 ...
 *
 * Models typically have few inputs and outputs, so tensors use a flat vector in
...
public:
   ...
   * @return A reference invalidated by a later insertion that grows the vector.
   */
  ...
  /** Return the named tensor, or `nullptr` when it is absent. */
  ...
  /**
   * @brief Return the named tensor.
   * @throws std::out_of_range If no tensor has `name`.
   */
