...
/**
 * @brief Process-wide registry shared by bundled and out-of-tree plugins.
 *
 * Registration throws for an empty name or null creator and returns `false`
 * for a duplicate name. Creation throws when a name is absent.
 *
 * @par Thread safety
 * Registration, lookup, creation, and enumeration may be called concurrently.
 */
...
public:
  ...
  /**
   * @brief Register a creator, returning `false` for a duplicate name.
   * @throws std::invalid_argument If the name is empty or creator is null.
   */
