...
/** Image/tensor dimensions in width-height-channel order. */
struct Shape {
  int w;
  int h;
  int c;
...
/** Configuration shared by the bundled frame preprocessors. */
...
struct FrameTransformContext {
  ...
  Shape origin_shape;
  Shape model_input_shape;
  /** Region consumed by preprocessing; the full frame when no ROI was supplied.
   */
  Rect roi;
  int top_pad = 0;
  int left_pad = 0;
