...
#ifdef WITH_TRT
TEST_F(YoloDetInferenceTest, TRTNormal) {
  AlgoConstructParams tempInferParams;
  inferParams.dataType = DataType::FLOAT32;
  inferParams.modelPath = (modelDir / "yolov11n_trt_fp16.engine").string();
  inferParams.name = "yolov11n";
  ...
  std::shared_ptr<InferBase> engine;
  engine = std::make_shared<TrtAlgoInference>(tempInferParams);
  ASSERT_NE(engine, nullptr);
  ASSERT_EQ(engine->initialize(), InferErrorCode::SUCCESS);
  ...
  cv::imwrite("vis_yolodet_trt.png", visImage);
}

TEST_F(YoloDetInferenceTest, TRTMultiThreads) {
  AlgoConstructParams tempInferParams;
  inferParams.dataType = DataType::FLOAT32;
  inferParams.modelPath = (modelDir / "yolov11n_trt_fp16.engine").string();
  inferParams.name = "yolov11n";
