...
// Pack an fp32 vector into an fp16 (uint16) TypedBuffer via a minimal
// IEEE-754 half conversion (finite, in-range values only).
...
TypedBuffer halfBuffer(const std::vector<float> &values) {
  std::vector<uint8_t> bytes(values.size() * sizeof(uint16_t));
  auto *halves = reinterpret_cast<uint16_t *>(bytes.data());
  for (size_t i = 0; i < values.size(); ++i) {
    halves[i] = floatToHalf(values[i]);
  }
  return TypedBuffer::createFromCpu(DataType::FLOAT16, std::move(bytes));
}

// Batch identity context with N slots.
std::shared_ptr<RuntimeContext> identityBatchContext(int w, int h, int n) {
  ...
  ctx->frame_transform_batch.assign(n, t);
  return ctx;
}
...
TEST(RawOutputDecode, EmptyOutputFails) {
  TensorData model_output; // empty
  GenericPostParams params;
  AlgoPostprocParams post_params;
  post_params.setParams(params);
...
TEST(RawOutputDecode, BatchIsUnsupported) {
  TensorData model_output;
  model_output.set("feat", floatBuffer({1.f}), {1, 1});
  GenericPostParams params;
  AlgoPostprocParams post_params;
  post_params.setParams(params);
  ...
  auto ctx = identityContext(4, 4);
  std::vector<AlgoOutput> outputs;
  EXPECT_EQ(postproc.batchProcess(model_output, outputs, ctx),
            InferErrorCode::InferOutputError);
}
...
TEST(YoloDetDecode, Fp16OutputPath) {
  const int nc = 1, anchors = 1;
  std::vector<float> data(static_cast<size_t>(4 + nc) * anchors, 0.f);
  data[0] = 100.f;
  data[1] = 100.f;
  ...
  TensorData model_output;
  model_output.set("output0", halfBuffer(data), {1, 4 + nc, anchors});
  ...
  ASSERT_NE(det, nullptr);
  ASSERT_EQ(det->bboxes.size(), 1u);
  EXPECT_EQ(det->bboxes[0].label, 0);
  EXPECT_EQ(det->bboxes[0].rect, (Rect{90, 90, 20, 20}));
}

TEST(YoloDetDecode, BatchDecodesEachImage) {
  const int nc = 1, anchors = 1, batch = 2;
  std::vector<float> data(static_cast<size_t>(batch) * (4 + nc) * anchors, 0.f);
  auto set = [&](int b, int attr, float v) {
    data[b * (4 + nc) * anchors + attr * anchors + 0] = v;
  };
  set(0, 0, 100.f);
  set(0, 1, 100.f);
  set(0, 2, 20.f);
  ...
  TensorData model_output;
  model_output.set("output0", floatBuffer(data), {batch, 4 + nc, anchors});
  ...
  auto ctx = identityBatchContext(640, 640, batch);
  std::vector<AlgoOutput> outputs;
  ASSERT_EQ(postproc.batchProcess(model_output, outputs, ctx),
            InferErrorCode::SUCCESS);

  ASSERT_EQ(outputs.size(), 2u);
  EXPECT_EQ(outputs[0].getParams<DetRet>()->bboxes.at(0).rect,
            (Rect{90, 90, 20, 20}));
  EXPECT_EQ(outputs[1].getParams<DetRet>()->bboxes.at(0).rect,
            (Rect{180, 180, 40, 40}));
}
...
TEST(FprClsDecode, BatchDecodesEachRow) {
  ...
  auto ctx = identityBatchContext(4, 4, 2);
  std::vector<AlgoOutput> outputs;
  ASSERT_EQ(postproc.batchProcess(model_output, outputs, ctx),
            InferErrorCode::SUCCESS);
  ASSERT_EQ(outputs.size(), 2u);
  EXPECT_EQ(outputs[0].getParams<FprClsRet>()->label, 1);
  EXPECT_EQ(outputs[1].getParams<FprClsRet>()->label, 0);
  EXPECT_EQ(outputs[1].getParams<FprClsRet>()->birad, 0);
}
...
TEST(OcrRecoDecode, BatchCtcPerRow) {
  ...
  ASSERT_EQ(postproc.batchProcess(model_output, outputs, ctx),
            InferErrorCode::SUCCESS);
  ASSERT_EQ(outputs.size(), 2u);
  EXPECT_EQ(outputs[0].getParams<OCRRecoRet>()->outputs,
            (std::vector<int64_t>{4, 5}));
  EXPECT_EQ(outputs[1].getParams<OCRRecoRet>()->outputs,
            (std::vector<int64_t>{7, 8}));
}

TEST(NanoDetDecode, BatchDecodesEachImage) {
  const int nc = 2, anchors = 1, batch = 2;
  const int row = nc + 4;
  std::vector<float> data(static_cast<size_t>(batch) * anchors * row, 0.f);
  // image 0: class 1, box (10,10)-(50,30)
  data[1] = 0.9f;
  data[nc + 0] = 10.f;
  data[nc + 1] = 10.f;
  data[nc + 2] = 50.f;
  data[nc + 3] = 30.f;
  // image 1: class 0, box (5,5)-(25,45)
  data[anchors * row + 0] = 0.8f;
  data[anchors * row + nc + 0] = 5.f;
  data[anchors * row + nc + 1] = 5.f;
  data[anchors * row + nc + 2] = 25.f;
  data[anchors * row + nc + 3] = 45.f;

  TensorData model_output;
  model_output.set("output", floatBuffer(data), {batch, anchors, row});
  ...
  ASSERT_EQ(postproc.batchProcess(model_output, outputs, ctx),
            InferErrorCode::SUCCESS);
  ASSERT_EQ(outputs.size(), 2u);
  EXPECT_EQ(outputs[0].getParams<DetRet>()->bboxes.at(0).label, 1);
  EXPECT_EQ(outputs[0].getParams<DetRet>()->bboxes.at(0).rect,
            (Rect{10, 10, 40, 20}));
  EXPECT_EQ(outputs[1].getParams<DetRet>()->bboxes.at(0).rect,
            (Rect{5, 5, 20, 40}));
...
TEST(RtmDetDecode, BatchDecodesEachImage) {
  const int nc = 2, anchors = 1, batch = 2;
  std::vector<float> det{
      10.f, 10.f, 60.f, 40.f, // image 0
  ...
  model_output.set("labels", floatBuffer(cls), {batch, anchors, nc});
  ...
  EXPECT_EQ(outputs[0].getParams<DetRet>()->bboxes.at(0).rect,
            (Rect{10, 10, 50, 30}));
  EXPECT_EQ(outputs[1].getParams<DetRet>()->bboxes.at(0).label, 0);
}

TEST(SemanticSegDecode, BatchProducesPerImageContours) {
  const int nc = 2, h = 4, w = 4, batch = 2;
  const size_t plane = static_cast<size_t>(nc) * h * w;
  std::vector<float> data(plane * batch, 0.f);
  auto fill = [&](int b) {
    float *img = data.data() + b * plane;
    for (int i = 0; i < h * w; ++i) {
      img[i] = 0.9f; // class 0 background
    }
    for (int y = 2; y < 4; ++y) {
      for (int x = 2; x < 4; ++x) {
        img[0 * h * w + y * w + x] = 0.1f;
        img[1 * h * w + y * w + x] = 0.9f;
      }
    }
  };
  fill(0);
  ...
  TensorData model_output;
  model_output.set("seg", floatBuffer(data), {batch, nc, h, w});
  ...
  auto ctx = identityBatchContext(w, h, batch);
  std::vector<AlgoOutput> outputs;
  ASSERT_EQ(postproc.batchProcess(model_output, outputs, ctx),
            InferErrorCode::SUCCESS);
  ASSERT_EQ(outputs.size(), 2u);
  for (auto &out : outputs) {
    ASSERT_TRUE(out.getParams<SegRet>()->cls_to_contours.count(1));
  }
}

TEST(UnetDualSegDecode, BatchReturnsPerImageTensors) {
  const int w = 2, h = 3, batch = 2;
  const size_t plane = static_cast<size_t>(w) * h;
  std::vector<float> prob(plane * batch);
  std::vector<float> mask(plane * batch);
  for (size_t i = 0; i < prob.size(); ++i) {
    prob[i] = 0.1f * static_cast<float>(i);
    mask[i] = static_cast<float>(i % 2);
  }
  ...
  TensorData model_output;
  model_output.set("prob", floatBuffer(prob), {batch, w, h});
  model_output.set("mask", floatBuffer(mask), {batch, w, h});
  ...
  auto ctx = identityBatchContext(8, 8, batch);
  std::vector<AlgoOutput> outputs;
  ASSERT_EQ(postproc.batchProcess(model_output, outputs, ctx),
            InferErrorCode::SUCCESS);
  ASSERT_EQ(outputs.size(), 2u);
  for (auto &out : outputs) {
    EXPECT_EQ(out.getParams<DualRawSegRet>()->prob.shape,
              (std::vector<int>{h, w}));
  }
}
