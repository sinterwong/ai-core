...
- [ ] **预处理/后处理异常泄漏**：`AlgoPreproc::Impl::process` / `batchProcess` 及 postproc 对应方法没有 try/catch，插件内部 `throw` 会穿透 `AlgoInference::infer`。facade 边界统一 catch-all 转错误码。
- [ ] **ORT 后端 `infer` / `terminate` 竞态**：`infer` 不持锁、`terminate` 持锁 reset session，并发即 use-after-free。统一锁策略。
- [ ] **初始化检查失效**：`AlgoInference::Impl::infer` 判空的三个成员指针在构造函数就创建了，永远非空。加 `m_initialized`，未初始化 fail fast。

### API 清算（一次性 break）

- [ ] 命名：`IPreprocssPlugin` → `IPreprocessPlugin`、`IPostprocssPlugin` → `IPostprocessPlugin`、`DaulRawSegRet` → `DualRawSegRet`、`accelerator_buffer_impl.hpp` 更名（它是接口不是 impl）。
...
- [ ] `escaleResizeWithPad` 坐标还原单测（检测框正确性的命门，且此逻辑不受 API 重塑影响，先锁住）。
- [ ] `ai_core-config.cmake.in` 加 `find_dependency`；`CMAKE_CUDA_ARCHITECTURES` 去硬编码；`install/` 移出仓库。
...
- [ ] **`ImageView` 抽象**：公共 API 以自有 `ImageView`（data ptr + shape + stride + format）替代 `cv::Mat` / `cv::Rect`，OpenCV 降为实现细节；`ai_core/opencv_interop.hpp` 提供零成本互转。这是支持非视觉模态（音频、文本、任意张量输入）的前提——"AI Core" 这个名字才名副其实。
- [ ] **`TensorData` v2**：`datas` 与 `shapes` 两个平行 map 易失同步，聚合为单一 `Tensor` 类型（buffer + shape + dtype）；容器换按插入序 small-vector + 名字查找（输入输出通常 1~3 个，`std::map` 纯浪费）。
- [ ] **`TypedBuffer` 收口**：工厂方法梳理，`resize` 的分歧语义（Pageable 保数据 / Pinned、GPU 破坏性）在类型层面显式化。
- [ ] **配置与数据分离**：不变的 preproc/postproc 参数在 `initialize` 时绑定并校验一次，`infer()` 只带数据（保留可选 per-call override）。
